defmodule EmaWeb.ExecutionController do
  ...
  def create(conn, params) do
    case Ema.Executions.create(params) do
      ...
      {:error, changeset} ->
        conn |> put_status(422) |> json(%{errors: Ecto.Changeset.traverse_errors(changeset, fn {msg, _} -> msg end)})
    end
  end
  ...
  def events(conn, %{"id" => id}) do
    ...
    json(conn, %{events: Enum.map(events, fn e ->
  ...
  def agent_sessions(conn, %{"id" => id}) do
    ...
    json(conn, %{agent_sessions: Enum.map(sessions, fn s ->
      %{
  ...
  defp serialize(e) do
    %{
      ...
      project_slug: e.project_slug, intent_slug: e.intent_slug,
      intent_path: e.intent_path, result_path: e.result_path,
      requires_approval: e.requires_approval,
      brain_dump_item_id: e.brain_dump_item_id,
      proposal_id: e.proposal_id, task_id: e.task_id, session_id: e.session_id,
      metadata: e.metadata, completed_at: e.completed_at,
      inserted_at: e.inserted_at, updated_at: e.updated_at
    }
  end
