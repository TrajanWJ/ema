defmodule Ema.MCP.DomainTools do
  ...
  def list do
    [
      ...
      tool(
        "ema_approve_proposal",
        "Approve a proposal — creates an execution",
        %{
          "id" => %{"type" => "string"}
        },
        ["id"]
      ...
      tool(
        "ema_kill_proposal",
        "Kill a proposal — records pattern in KillMemory",
        %{
          "id" => %{"type" => "string"},
          "reason" => %{"type" => "string"}
        },
      ...
      tool(
        "ema_redirect_proposal",
        "Redirect a proposal — creates 3 new seed angles",
        %{
          "id" => %{"type" => "string"},
          "note" => %{"type" => "string"}
        },
      ...
      tool(
        "ema_proposal_lineage",
        "Show proposal lineage tree (seeds, parents, children)",
        %{
          "id" => %{"type" => "string"}
        },
        ["id"]
      ...
      tool(
        "ema_vault_read",
        "Read a vault note by path. Optionally include link graph data.",
        %{
          "path" => %{"type" => "string", "description" => "Path relative to vault root"},
          "include_links" => %{
            "type" => "boolean",
            "description" => "Include backlinks and forward links (default false)"
          }
        },
        ["path"]
      ),
      ...
      tool(
        "ema_vault_graph",
        "Vault link graph — stats, neighbors, typed neighbors, or orphans",
        %{
          "mode" => %{
            "type" => "string",
            "description" => "stats (default), neighbors, typed_neighbors, or orphans",
            "enum" => ["stats", "neighbors", "typed_neighbors", "orphans"]
          },
          "note_id" => %{
            "type" => "string",
            "description" => "Note ID (required for neighbors/typed_neighbors modes)"
          }
        }
      ),
      ...
      tool(
        "ema_show_goal",
        "Get goal details with children",
        %{
          "id" => %{"type" => "string"}
        },
        ["id"]
      ...
      tool(
        "ema_update_goal",
        "Update a goal's title, status, or description",
        %{
          "id" => %{"type" => "string"},
          "title" => %{"type" => "string"},
          "status" => %{"type" => "string"},
      ...
      tool(
        "ema_process_brain_dump",
        "Process a brain dump item into task, journal, archive, or note",
        %{
          "id" => %{"type" => "string"},
      ...
      tool(
        "ema_check_in_responsibility",
        "Check in on a responsibility",
        %{
          "id" => %{"type" => "string"},
          "notes" => %{"type" => "string"},
      ...
      tool(
        "ema_execution_events",
        "List execution lifecycle events",
        %{
          "id" => %{"type" => "string"}
        },
        ["id"]
      ...
      tool(
        "ema_create_pipe",
        "Create an automation pipe",
        %{
          "name" => %{"type" => "string"},
          "trigger" => %{
            "type" => "string",
            "description" => "Trigger pattern (e.g. brain_dump:created)"
          },
          "description" => %{"type" => "string"}
        },
        ["name"]
      ...
      tool(
        "ema_babysitter_nudge",
        "Send nudge message to babysitter",
        %{
          "message" => %{"type" => "string"}
        },
        ["message"]
      ...
      tool(
        "ema_advance_phase",
        "Transition an actor to a new phase",
        %{
          "id" => %{"type" => "string"},
      ...
      tool(
        "ema_list_phases",
        "List phase transitions for an actor",
        %{
          "id" => %{"type" => "string"}
        },
        ["id"]
      ...
      tool(
        "ema_actor_velocity",
        "Compute actor velocity — weeks completed and avg transitions",
        %{
          "id" => %{"type" => "string"}
        },
        ["id"]
      ...
      tool(
        "ema_tag_entity",
        "Tag an entity (task, project, etc.)",
        %{
      ...
      tool(
        "ema_set_entity_data",
        "Set per-actor metadata on an entity",
        %{
          "entity_type" => %{"type" => "string"},
          "entity_id" => %{"type" => "string"},
          "actor_id" => %{"type" => "string"},
      ...
      tool(
        "ema_get_entity_data",
        "Get per-actor metadata for an entity",
        %{
          "entity_type" => %{"type" => "string"},
          "entity_id" => %{"type" => "string"},
          "actor_id" => %{"type" => "string"}
      ...
      tool(
        "ema_delete_entity_data",
        "Delete per-actor metadata from an entity",
        %{
          "entity_type" => %{"type" => "string"},
          "entity_id" => %{"type" => "string"},
          "actor_id" => %{"type" => "string"},
      ...
      # ── Container Config ──
      tool(
        "ema_set_container_config",
        "Set config on a container (space/project/task)",
        %{
      ...
      tool(
        "ema_get_container_config",
        "Get config for a container",
        %{
          "container_type" => %{"type" => "string"},
          "container_id" => %{"type" => "string"}
        },
  ...
  # Proposals
  def call("ema_get_proposals", args, _),
    do: get("/proposals", pick(args, ~w(status project_id limit)))

  def call("ema_show_proposal", %{"id" => id}, _), do: get("/proposals/#{id}")
  def call("ema_approve_proposal", %{"id" => id}, _), do: post("/proposals/#{id}/approve")

  def call("ema_kill_proposal", %{"id" => id} = a, _),
    do: post("/proposals/#{id}/kill", pick(a, ~w(reason)))

  def call("ema_redirect_proposal", %{"id" => id} = a, _),
    do: post("/proposals/#{id}/redirect", pick(a, ~w(note)))
  ...
  # Goals
  def call("ema_create_goal", args, _),
    do: post("/goals", pick(args, ~w(title description status timeframe parent_id project_id)))

  def call("ema_show_goal", %{"id" => id}, _), do: get("/goals/#{id}")

  def call("ema_update_goal", %{"id" => id} = a, _),
    do: put("/goals/#{id}", pick(a, ~w(title status description timeframe)))
  ...
  def call("ema_process_brain_dump", %{"id" => id} = a, _),
    do: patch("/brain-dump/items/#{id}/process", pick(a, ~w(action)))
  ...
  def call("ema_toggle_habit", %{"id" => id} = a, _),
    do: post("/habits/#{id}/toggle", pick(a, ~w(date)))
  ...
  def call("ema_create_seed", args, _),
    do: post("/seeds", pick(args, ~w(title prompt type project_id)))
  ...
  # Responsibilities
  def call("ema_get_responsibilities", args, _),
    do: get("/responsibilities", pick(args, ~w(project_id)))
  ...
  # Projects
  def call("ema_create_project", args, _),
    do: post("/projects", pick(args, ~w(name slug path description)))
  ...
  def call("ema_create_pipe", args, _),
    do: post("/pipes", pick(args, ~w(name trigger description)))
  ...
  def call("ema_babysitter_nudge", %{"message" => msg}, _),
    do: post("/babysitter/nudge", %{"message" => msg})
  ...
  defp http(method, path, body \\ nil) do
    ...
    opts =
      if method in [:post, :put, :patch], do: Keyword.put(opts, :json, body || %{}), else: opts
