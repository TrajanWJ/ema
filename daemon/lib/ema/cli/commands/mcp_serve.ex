defmodule Ema.CLI.Commands.McpServe do
  ...
  defp all_tools do
    [
      ...
      # Brain Dump
      tool("ema_brain_dump", "Add to brain dump inbox", %{"content" => %{"type" => "string"}}, [
        "content"
      ]),
      ...
      tool("ema_delete_brain_dump", "Delete brain dump item", %{"id" => %{"type" => "string"}}, [
        "id"
      ]),
      ...
      tool("ema_proposal_lineage", "Show proposal lineage", %{"id" => %{"type" => "string"}}, [
        "id"
      ]),
      ...
      tool("ema_execution_events", "List execution events", %{"id" => %{"type" => "string"}}, [
        "id"
      ]),
      ...
      tool("ema_toggle_seed", "Toggle seed active/paused", %{"id" => %{"type" => "string"}}, [
        "id"
      ]),
      ...
      tool(
        "ema_spawn_session",
        "Spawn Claude Code session with EMA context",
        %{
          "prompt" => %{"type" => "string"},
      ...
      tool("ema_session_context", "Get project context bundle", %{
        "project_slug" => %{"type" => "string"}
      }),
      tool(
        "ema_resume_session",
      ...
      tool("ema_kill_session", "Kill a session", %{"session_id" => %{"type" => "string"}}, [
        "session_id"
      ]),
      ...
      tool(
        "ema_orient",
        "Get current EMA state and orientation",
        %{
      ...
      tool(
        "ema_phase_status",
        "Current phase + transition history for an actor",
        %{
          "actor_slug" => %{"type" => "string"}
        },
        ["actor_slug"]
      ...
      tool(
        "ema_sprint_cycle",
        "Start/review/complete planning cycles (week/month/quarter)",
        %{
          "actor_slug" => %{"type" => "string"},
      ...
      tool(
        "ema_workspace",
        "Actor workspace state: data, tags, config",
        %{
          "op" => %{"type" => "string"},
          "actor_slug" => %{"type" => "string"},
      ...
      tool(
        "ema_dispatch",
        "Dispatch an execution for agent work",
        %{
          "title" => %{"type" => "string"},
      ...
      tool(
        "ema_codebase_ask",
        "Query codebase via knowledge graph + vault",
        %{
          "query" => %{"type" => "string"},
          "project_slug" => %{"type" => "string"}
        },
  ...
  defp call_tool("ema_transition_task", %{"id" => id} = a),
    do: api_post("/tasks/#{id}/transition", Map.delete(a, "id"))
  ...
  defp call_tool("ema_process_brain_dump", %{"id" => id} = a),
    do: api_patch("/brain-dump/items/#{id}/process", Map.delete(a, "id"))

  defp call_tool("ema_delete_brain_dump", %{"id" => id}),
    do: api_delete("/brain-dump/items/#{id}")

  # Vault
  defp call_tool("ema_search_vault", %{"query" => q} = a),
    do: api_get("/vault/search", %{"q" => q, "space" => a["space"]})
  ...
  defp call_tool("ema_vault_write", %{"path" => _, "content" => _} = a),
    do: api_put("/vault/note", a)
  ...
  defp call_tool("ema_update_goal", %{"id" => id} = a),
    do: api_put("/goals/#{id}", Map.delete(a, "id"))
  ...
  defp call_tool("ema_approve_proposal", %{"id" => id}),
    do: api_post("/proposals/#{id}/approve", %{})

  defp call_tool("ema_kill_proposal", %{"id" => id} = a),
    do: api_post("/proposals/#{id}/kill", Map.delete(a, "id"))

  defp call_tool("ema_redirect_proposal", %{"id" => id} = a),
    do: api_post("/proposals/#{id}/redirect", Map.delete(a, "id"))
  ...
  defp call_tool("ema_approve_execution", %{"id" => id}),
    do: api_post("/executions/#{id}/approve", %{})

  defp call_tool("ema_cancel_execution", %{"id" => id}),
    do: api_post("/executions/#{id}/cancel", %{})
  ...
  defp call_tool("ema_toggle_habit", %{"id" => id} = a),
    do: api_post("/habits/#{id}/toggle", Map.delete(a, "id"))
  ...
  defp call_tool("ema_journal_write", %{"content" => _} = a),
    do: api_put("/journal/#{a["date"] || today()}", a)

  defp call_tool("ema_journal_search", %{"query" => q}),
    do: api_get("/journal/search", %{"q" => q})
  ...
  defp call_tool("ema_chat_agent", %{"slug" => s, "message" => m}),
    do: api_post("/agents/#{s}/chat", %{"message" => m})

  # Responsibilities
  defp call_tool("ema_get_responsibilities", args), do: api_get("/responsibilities", args)

  defp call_tool("ema_check_in_responsibility", %{"id" => id} = a),
    do: api_post("/responsibilities/#{id}/check-in", Map.delete(a, "id"))
  ...
  defp call_tool("ema_babysitter_nudge", %{"message" => m}),
    do: api_post("/babysitter/nudge", %{"message" => m})
  ...
  defp call_tool("ema_check_session", %{"session_id" => id}),
    do: api_get("/orchestrator/sessions/#{id}/check")
  ...
  defp call_tool("ema_kill_session", %{"session_id" => id}),
    do: api_post("/orchestrator/sessions/#{id}/kill", %{})

  # Context
  defp call_tool("context_operator", _), do: api_get("/context/operator/package")

  defp call_tool("context_project", %{"project" => ref}),
    do: api_get("/context/project/#{ref}/package")
