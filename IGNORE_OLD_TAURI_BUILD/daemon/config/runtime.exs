...
api_tokens =
  [
    System.get_env("EMA_API_ACCESS_TOKEN"),
    System.get_env("EMA_API_TOKEN")
  ]
  |> Kernel.++(String.split(System.get_env("EMA_API_ACCESS_TOKENS") || "", ",", trim: true))
  |> Enum.map(&String.trim/1)
  |> Enum.reject(&(&1 == ""))
  |> Enum.uniq()

if api_tokens != [] do
  config :ema, :api_access_tokens, api_tokens
end

allowed_origins =
  System.get_env("EMA_ALLOWED_ORIGINS")
  |> case do
    nil -> []
    value -> String.split(value, ",", trim: true) |> Enum.map(&String.trim/1)
  end

if allowed_origins != [] do
  config :ema, :api_allowed_origins, allowed_origins
  config :ema, :websocket_allowed_origins, allowed_origins
end
